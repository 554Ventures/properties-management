...
- **Tests**: 66 backend (incl. SSE protocol, pause/resume across restart, MCP in-process client, audit attribution, full-CRUD archive/lifecycle + archived-money treatment, Supabase-mode auth + cron endpoint) + 74 frontend (incl. axe smoke over all block types + all CRUD modals + Login, allowlist, answer-failure recovery, date-input UTC round-trip) — all green; `npm run build` clean.
