...
| `/money/review` | Review queue (§5.4) | Pending bank/receipt transactions w/ AI category suggestion chip (never auto-applied), confirm/override; rent-match chip for income deposits that look like an open expected rent (accepting links the deposit and marks the RentPayment paid); property/unit attribution selects on the confirm card. |
