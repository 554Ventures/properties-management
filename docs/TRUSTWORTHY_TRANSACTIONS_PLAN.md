...
> Status: proposed (2026-07-12). Scope confirmed with the account owner. Not yet implemented — tracked in `WHATS_NEXT.md` §3.
...
- Backend: add a lightweight read used by the Rent page — for each open/partial charge, detect **Rent-categorized income transactions** (category name "Rent", not yet linked to a deposit) on the same property/unit within the period window that could apply. Reuse `findRentMatchCandidates`/`pickRentMatch` (`rent.service.ts` 255-306), relaxed to partial and returning near-matches.
