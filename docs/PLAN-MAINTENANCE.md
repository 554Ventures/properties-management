...
*Authored 2026-08-13 from WHATS_NEXT §4 "Maintenance / work orders — no home in the app", which asks for a design pass against PRD scope before scheduling. **Phase 1 is buildable now; the PRD question (D1) is the owner's call and is the one thing that gates starting.***
...
1. **D1 — PRD amendment (blocking).** Ratify as approved product scope before Phase 1, or build under the "contractor directory shipped ahead of the PRD" precedent? *Recommendation: ratify* — the tenant portal's dependency on this record should be visible in the document that governs the portal. **Amendment opened 2026-08-13 and awaiting the owner's sign-off**: [PRD.md](PRD.md) carries proposed amendment **A1**, every inserted block marked `[A1]`, touching §4 (IA row), §5.10 (new feature spec), §8 (`WorkOrder` + `Contractor` rows), §11 (fourth forward-compatibility bullet), §14 (Phase 2). It ratifies `Contractor` alongside `WorkOrder`, since ratifying the new entity while leaving the shipped one unratified would repeat the very gap this plan names. Ratify by deleting the banner and the `[A1]` markers; reject by deleting the marked blocks and recording it here. Note that A1 also *surfaces but deliberately does not resolve* three further §4 drifts (retired Tenants & Leases, retired AI Insights, added Documents) — quietly rewriting the table would erase that its "core usability contract" was overridden four times.
...
5. **D5 — Does `logJob` survive?** Once a work order can carry cost, `POST /contractors/:id/jobs` is a second path to the same outcome, and it mints a confirmed expense behind a `properties` guard. Options: leave it, re-gate it to `money`, or fold it into the work-order flow. *Recommendation: re-gate to `money` now, independently of this plan* — it is a live permission gap, not a design question.
