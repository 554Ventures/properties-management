...
│           └── pages/            # Dashboard, PropertiesList, PropertyDetail,
...
**Transaction** — `id`, `accountId`, `propertyId String?`, `unitId String?`, `categoryId String?`, `date DateTime`, `amountCents Int` (always positive; sign from `type`), `type String` (TransactionType), `description String`, `vendor String?`, `source String` (TransactionSource), `status String` (TransactionStatus), `classification String?` (TransactionClassification: `transfer | owner_contribution | refund`; null = ordinary — transfers/owner money are excluded from every P&L/KPI aggregate and refunds net as negative expense against their category, all via `lib/pnl.ts`; rows stay visible in ledger/GL), `aiSuggestedCategoryId String?`, `aiConfidence Float?`, `receiptUrl String?`, `createdAt`, `updatedAt`. Indexes: `@@index([accountId, date])`, `@@index([accountId, status])`. Central ledger; all reports derive from confirmed rows.

**RentPayment** — `id`, `leaseId`, `period String` ("YYYY-MM"), `dueDate DateTime` (materialized when the month's expected rows are created; enables exact days-late), `amountCents Int`, `paidCents Int @default(0)` (running total received = Σ deposits), `method String?` (RentPaymentMethod, null until paid), `status String` (RentPaymentStatus), `paidAt DateTime?`, `externalRef String?` (mock Stripe id), `transactionId String? @unique` (legacy single-payment link; deposits are the source of truth), `remindedAt DateTime?`. `@@unique([leaseId, period])`.

**RentPaymentDeposit** — `id`, `rentPaymentId`, `transactionId String @unique` (each deposit backed 1:1 by a ledger Transaction), `amountCents Int`, `tenantId String?` (which co-tenant paid), `method String?`, `paidAt DateTime`. Many deposits sum toward one charge (partial/roommate payments); every deposit write recomputes the parent's `paidCents` in the same transaction, and `DELETE /rent/payments/:id/deposits/:depositId` is the sanctioned unlink path (the ledger Transaction survives).
...
| GET `/properties/:id` | — → `PropertyDetailResponse` (property, units w/ lease+tenant+status + `rent` (this-month snapshot: period/status/daysLate/paid/amount/dueDate — derived read-only, synthesized in memory when no charge row exists yet, never persisted) + `leaseCount` + `pendingLease`, `pnl: PnlSummary` MTD/YTD, `insights: Insight[]`) |
...
**Leases** — GET `/leases?status`; GET `/leases/:id` → `LeaseDetailResponse` (`lease: LeaseWithContext` — unitLabel/propertyLabel/tenants — + `rentPayments`); POST `/leases` (`CreateLeaseInput`: unitId, tenantIds, rentCents, dueDay, start/end); PATCH `/leases/:id`; POST `/leases/:id/terminate` → `Lease` (status→ended); POST `/leases/:id/tenants` (`AddLeaseTenantInput`, incl. `shareCents?`) and DELETE `/leases/:id/tenants/:tenantId` → `LeaseWithContext` (co-tenants; can't remove last, primary auto-promotes); PATCH `/leases/:id/tenants/:tenantId` (`UpdateLeaseTenantShareInput` — set/clear a co-tenant's expected share; null = even-split fallback) → 204; POST `/leases/:id/renewal` (`AcceptRenewalInput`) → `Lease` (immediate switchover: new active lease, source ended); POST `/leases/:id/renewal-draft` → `RenewalDraftResponse` (proposed terms incl. `suggestedRentCents` from market-rent heuristic); POST `/leases/:id/esign` → `EsignEnvelopeResponse` (mock Docusign envelope + status). New management schemas live in `schemas/lease-management.ts`.

**Transactions**
| Method/Path | Request → Response |
|---|---|
| GET `/transactions?from&to&propertyId&type&status&categoryId&cursor&limit` | → `TransactionListResponse` (items + `nextCursor`) |
| POST `/transactions` | `CreateTransactionInput` (incl. `classification?`) → `Transaction` (if `categoryId` omitted, response carries `aiSuggestedCategoryId`+`aiConfidence` for the "AI guess" chip — per-account vendor memory consulted before the keyword table; save always explicit) |
| PATCH `/transactions/:id` | `UpdateTransactionInput` → `Transaction` |
| DELETE `/transactions/:id` | → 204 |
| GET `/transactions/review` | → `ReviewQueueResponse` (pending_review items w/ suggestions; `suggestionSource: 'learned'` when the account's vendor memory backs the pick, `rentMatch` targeting the charge's remaining balance, `possibleDuplicate` fingerprint flag) |
| POST `/transactions/:id/confirm` | `ConfirmTransactionInput` (`categoryId?` override, `rentPaymentId?` deposit link, `propertyId?`/`unitId?`, `classification?`) → `Transaction` |
...
| Method/Path | Request → Response |
|---|---|
| GET `/rent/tracker?period=YYYY-MM` | → `RentTrackerResponse`: `{ period, collectedCents, outstandingCents, paidUnits, partialUnits, totalUnits, rows: RentTrackerRow[] }` (row: tenant, unit/property, amountCents, `paidCents`, dueDate, status incl. `partial`, `daysLate?`, method?, paidAt?, `deposits: RentDeposit[]`, `tenants: RentTenantShare[]` + `sharesMismatch`) |
| GET `/rent/unlinked-deposits?period=` | → `{ items: UnlinkedRentDeposit[] }` — confirmed Rent-categorized income fitting exactly one still-open charge (≤ remaining, in-window, attribution-compatible; ambiguous suppressed); the Rent page's "Link to rent?" nudge |
| POST `/rent/payments` | `RecordRentPaymentInput` (leaseId, period, amountCents ≤ remaining, method, paidAt?, `tenantId?` attribution) → `RentPayment` (writes ledger Transaction + RentPaymentDeposit; below-remaining = partial) |
| DELETE `/rent/payments/:id/deposits/:depositId` | → `RentPayment` — unlink one deposit; recomputes `paidCents`/status, ledger Transaction survives (audited `rent_payment.deposit_unlinked`) |
...
**Insights** — GET `/insights?status&scope` → `Insight[]` (each may carry a structured `action: InsightAction` — label + `api_call`/`navigate`, the chat action-card vocabulary; api_call paths must be on the web action allowlist); POST `/insights/:id/dismiss` → `Insight`; POST `/insights/:id/actioned` → `Insight` (user executed the suggested action; audited `ai_suggested_user_confirmed`, actor fixed server-side); GET `/insights/monthly-reviews` → `Report[]` (type `monthly_review`); GET `/insights/monthly-reviews/:id` → `ReportDetailResponse`; POST `/insights/monthly-reviews/generate` → `Report` (dev/demo trigger for the scheduled job).
...
- **leaseService** — `list(accountId, filter?)` R A (excludes archived unit/property) · `getDetail(accountId, id)` R A · `create/update` R · `terminate(accountId, id)` R (also reconciles the shortened lease's open due/failed charges: months no longer covered are voided, the final month re-prorates to occupied days; paid/processing rows untouched, adjustments audited) · `addTenant/removeTenant(accountId, leaseId, …)` R (last-tenant guard, primary auto-promote) · `createRenewal(accountId, leaseId, input)` R (switchover: new active lease, source ended; the source's open charges reconcile in the same transaction — the switchover month's unpaid charge becomes the blended old-share + new-share proration, so the unit is never billed twice for one month) · `draftRenewal(accountId, leaseId)` R A (returns proposal; sending is separate) · `sendForEsign(accountId, leaseId)` R (mock Docusign). All writes audited.
- **transactionService** — `list(accountId, filter)` R A M · `create(accountId, input)` R A(write) M(write) · `update/remove` R · `getReviewQueue(accountId)` R A · `confirm(accountId, id, categoryId?)` R A(write) M(write, as `categorize_transaction`) · `suggestCategory(accountId, partialTxn)` internal (per-account `VendorCategoryMemory` first — corrections stick, keyed on `categoryMemoryKey` — the vendor, or the bank descriptor when the feed carries no vendor (Stripe FC), with per-transaction noise (tagged reference ids, dates, check numbers) stripped; a key that names only a payment mechanism is refused rather than pooled; then keyword table: "plumb|roof|repair"→Repairs, "electric|water|gas"→Utilities, "insur"→Insurance, else Supplies @ 0.62 / income "Rent" @ 0.5 — deliberately below the 0.7 bulk-confirm gate) · `scanReceipt(accountId, image)` R (mock returns fixture parse) · `importFromBank(accountId)` R (mock Plaid → pending_review rows)
- **categoryService** — `list` R A M · `create` R
- **rentService** — `getMonthStatus(accountId, period)` R A M (materializes missing expected RentPayment rows per active lease for `period` — one charge per unit-month: a unit whose period already has a row from any lease is skipped; `amountCents` prorated by covered days when the lease spans only part of the month, else full rent; `dueDate = max(periodStart + dueDay − 1, lease.startDate)` — then derives) · `recordPayment(accountId, input)` R A(write) M(write) · `createPaymentLink(accountId, rentPaymentId)` R (mock Stripe) · `sendReminders(accountId, rentPaymentIds)` R A(write) M(write) — sets `remindedAt`, mock email, audit log
...
- **Rent status** (per RentPayment for the viewed period): `paid` if status `paid` **or** `paidCents ≥ amountCents`; `processing`/`failed` pass through; else `partial` if `0 < paidCents < amountCents` (with `daysLate` once past `dueDate + graceDays` — partially paid rent can still be late); else `due` if `today ≤ dueDate + account.graceDays` (v1 default 0), else `late` with `daysLate = floor(today − dueDate)` — always rendered as text ("6 days late", "Partial — $X of $Y"), never color alone. `paidCents` = Σ `RentPaymentDeposit.amountCents` for the charge (many deposits sum toward one charge; stored status flips to `paid` only when fully covered). Tracker `collectedCents = Σ paidCents` (partials included), `outstandingCents = Σ max(0, amountCents − paidCents)`, `collected% (by units) = paidUnits / totalActiveLeaseUnits` with `partialUnits` reported alongside.
- **KPIs** (`DashboardKpisResponse`): `netCashFlowMtdCents = Σ confirmed income − Σ confirmed expense (current month)` under `lib/pnl.ts` classification semantics (transfers/owner contributions excluded, refunds net as negative expense — applies to every money aggregate: KPIs, series, breakdown, NOI, P&L/cashflow/Schedule E/GL totals, monthly review, insights, tax set-aside); `rentCollectedPct = paidUnits/totalUnits` + `paidUnits`, `totalUnits`; `expensesMtdCents`; each with `trend` = pct change vs. same day-of-month window of prior month.
- **Tax set-aside** (estimate only; UI carries the PRD §13.4 disclaimer): `currentCents = round(confirmedNetMtdCents × taxRatePct/100)`; `targetCents = round(avgMonthlyNet × 3 × taxRatePct/100)` (a quarterly target), where `avgMonthlyNet` = trailing-6-full-month net ÷ the number of those months with any confirmed ledger activity (min 1, so a young account's target isn't understated). With seed data (§10): current $1,690, target $2,700.
- **Tenant/lease status**: `late` if any unpaid rent past due (including partially paid — daysLate presence covers both); else `renew_soon` if `lease.endDate ≤ today + RENEW_SOON_DAYS` (60 — shared constant in `@hearth/shared` consumed by tenant status, the renewal_window insight, and the web's renewal badges); else `current`.
- **Contractor usage stats** (`ContractorListRow`, `ContractorDetailResponse` — the detail's job history and stats derive from the same set, so both surfaces always agree): `jobsCount`/`avgCostCents`/`lastUsedAt` are never stored — they derive from confirmed, ordinary (classification-null) expense transactions **linked via `Transaction.contractorId`**; `avgCostCents = round(total/count)`; no linked history → `jobsCount 0` with `avgCostCents`/`lastUsedAt` null. The link itself is assigned at WRITE time, never re-derived at read time (*2026-08-03*, closing the "stats ride the vendor string" gap): wherever `vendor` is written (manual create/edit, bank import, bank-sync restatement) the row links to the single active contractor whose `name` matches the vendor case/whitespace-insensitively (`vendorKey = trim().toLowerCase()`; zero or several matches link nothing — the vendor→category memory uses the noisier-input `categoryMemoryKey` variant instead), a manually logged job links explicitly to its contractor, and creating/renaming/restoring a contractor adopts matching unlinked rows (never rows already linked). Renaming a contractor or editing an unrelated field therefore keeps history; editing a row's vendor deliberately re-derives that row's link (vendor is the signal for which contractor was paid).
- **Mock insight generation rules** (run by seed + `generateInsights`, deduped on `dedupeKey`; each emits a legacy route `actionTarget` — also the push deep link — plus a structured `action`): (1) `late_rent` — any payment (fully unpaid or partial) `daysLate > 5` → warning with the *remaining* amount in the body, link "Review" → `/rent?period=…`, action "Send reminder" = api_call `POST /rent/reminders {rentPaymentIds}` (reminders also dun for the remaining amount); (2) `expense_spike` — a category's current-month total > 125% of its trailing-3-month avg → warning, "View transactions" → `/money?type=expense&categoryId=…[&propertyId=…]`; (3) `renewal_window` — leases ending ≤ RENEW_SOON_DAYS → info (title interpolates the constant), "Review renewals" → `/tenants/:id` of the soonest-ending lease's primary tenant (home of the renewal-draft flow; the body names every renewing tenant); (4) `underperforming_property` — property net (trailing 3 mo) < 80% of per-unit portfolio average → info, "View property" → `/properties/:id`; (5) `contractor_cost_spike` — a contractor's most recent vendor-matched job landed this month and cost > 150% of their average across ≥ 3 prior jobs (same derivation as the contractor directory) → info, "View contractor" → `/maintenance/contractors/:id`; (6) `transactions_pending_review` — any `pending_review` transactions → info, "Review transactions" → `/money/review`. Rule 6 is a living count with an explicit lifecycle (unlike the monthly-keyed rules): its `dedupeKey` carries the newest pending row's id so a dismissal sticks only until the next import lands, an active card's count refreshes in place as rows are confirmed, and the card auto-resolves (status `actioned`) once the queue is cleared or superseded. Dashboard shows every active insight as a deck of cards (highest severity first, then newest; cycle controls); `GET /dashboard/insight` (single top card) remains for contract/MCP stability.
...
**Write tools** — 23 of them, covering transactions, properties, units, tenants, leases, rent, reports and insights. `ai/tools.ts` is the authoritative list; this doc deliberately does not enumerate them (the enumeration here went stale twice). Each call writes an AuditLog row with actor `system`, and each must be mapped in `WRITE_TOOL_PERMISSIONS` — an unmapped write tool is reachable by any member holding `ai`, bypassing its route's `requirePermission` guard. Gating differs per transport: stdio registers writes only when `HEARTH_MCP_ENABLE_WRITE=true`; the remote surface always registers them and narrows per request by the caller's permissions. Descriptions must state side effects plainly (e.g. "Sends a reminder email to the tenant — irreversible"). No `ask_user_question` on this surface (PRD §10 non-goal).

---

## 8. Frontend

**Routes** (react-router, all inside `AppShell`): `/` Dashboard · `/properties` · `/properties/:id` · `/tenants/:id` · `/money` (+ `/money/new`, `/money/review`) · `/rent` · `/reports` · `/reports/:id` · `/settings`. The standalone AI Insights page is retired — `/insights[/:reportId]` redirects to `/reports[/:id]` (monthly reviews render in the report viewer inside `AiSurface`), and insight cards surface contextually (Dashboard, PropertyDetail, RentTracker, Money). The standalone tenants list is retired too (2026-07-13) — `/tenants` redirects to `/properties`; tenants are reached via Properties → Unit, lease-less tenants via the "Tenants without a lease" section on `/properties`. Chat drawer is layout state (`?chat=open` for deep-linking, e.g. from "Ask AI to build a custom report" which opens it with report context), **not** a route (PRD §4).
...
- **AI-surface convention (PRD §6, mandatory):** `--surface-ai` (subtle cool tint), `--border-ai` (steel-blue→teal family; AI cards keep the plain card surface — the gradient BORDER via `--ai-border-gradient` + the `.ai-frame` utility is the sole marker), `--text-ai-label`; consumed by exactly one wrapper component `AiSurface` used by InsightCard, AiChip, all assistant chat bubbles, and monthly-review content — any AI-authored content is visually marked, everywhere, via this one component.
...
**Insights (active):** `late_rent:…okafor:<period>` (warning, dashboard card, link → `/rent?period=…`, action "Send reminder"); `expense_spike:utilities:birch:<period>` (warning); `renewal_window:<period>` (info, "2 leases up for renewal…"); `transactions_pending_review:<newest pending txn id>` (info, "3 imported transactions are waiting for review" → `/money/review`). **Monthly review:** one `monthly_review` Report for last month (bottom line, per-property net table, 2–3 watch items) so the Reports archive isn't empty of reviews. **Integrations:** 4 rows (plaid/stripe/docusign/email), status `mock`. Seed is idempotent (wipe-and-recreate for the demo account).
