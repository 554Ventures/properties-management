...
**Routes** (react-router, all inside `AppShell`): `/` Dashboard · `/properties` · `/properties/:id` · `/tenants` · `/tenants/:id` · `/money` (+ `/money/new`, `/money/review`) · `/rent` · `/reports` · `/reports/:id` · `/settings`. The standalone AI Insights page is retired — `/insights[/:reportId]` redirects to `/reports[/:id]` (monthly reviews render in the report viewer inside `AiSurface`), and insight cards surface contextually (Dashboard, PropertyDetail, TenantsList, RentTracker, Money). Chat drawer is layout state (`?chat=open` for deep-linking, e.g. from "Ask AI to build a custom report" which opens it with report context), **not** a route (PRD §4).
