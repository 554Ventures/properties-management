...
**Approach summary.** A three-package npm-workspaces monorepo where `packages/shared` (Zod) is the frozen contract both `apps/api` (Fastify + Prisma/SQLite) and `apps/web` (React/Vite/Tailwind/TanStack Query) compile against. All business rules live in `apps/api/src/services/*` — REST controllers, the chat agent's tool handlers, and the MCP server are three thin adapters over the same functions (PRD §9.2). The MCP server is a **second entrypoint inside `apps/api`** (`src/mcp/`), not a separate package: it needs the Prisma client, the service layer, and the seeded DB, and a thin `apps/mcp` package would only re-export those with extra wiring; the `@modelcontextprotocol/sdk` dependency is small and stdio transport needs no HTTP server. AI runs through a single `AiClient` interface with a real Anthropic implementation (`claude-sonnet-5`, override via `ANTHROPIC_MODEL`) and a deterministic `MockAiClient` used whenever `ANTHROPIC_API_KEY` is unset, so the entire app demos offline. Auth is a single seeded demo account with an optional static bearer token (`DEV_BEARER_TOKEN`); every service function takes `accountId` as its first argument so real auth slots in later without touching business logic (intentional deviation from PRD §7.3 for the first build).
...
│   │       ├── plugins/          # auth.ts (bearer/localhost), zod-validation.ts, error-handler.ts, sse.ts
...
**Conventions (binding):** SQLite provider does not support Prisma `enum` or `Decimal` — all enums are `String` columns validated by Zod enums in `@hearth/shared` (comment each column with its allowed values so the future Postgres migration adds real enums), and **all money is integer cents** (`Int`, suffix `Cents`). All ids are `cuid()`. Every root entity carries `accountId` for future multi-tenancy.
...
- **SQLite constraints** (no enums/Decimal) are handled by string+cents conventions, but any implementer adding a Prisma `enum` or `Decimal` breaks dev — the schema file must carry a header comment banning both.
