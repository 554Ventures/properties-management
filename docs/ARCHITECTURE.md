...
- **Tenant/lease status**: `late` if any unpaid rent past due (including partially paid — daysLate presence covers both); else `renew_soon` if `lease.endDate ≤ today + 60d`; else `current`.
