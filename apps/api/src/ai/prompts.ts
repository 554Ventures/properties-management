...
export function buildSystemPrompt(ctx: SystemPromptContext): string {
  const lines = [
    ...
    '- You can add and edit properties, tenants, and transactions this way. Gather the details you need from the user, then propose_action with an api_call button carrying the full request body. You may never delete records.',
    ...
    '  • All money is integer cents. Before proposing an EDIT, fetch the record first (get_property / get_tenant / list_transactions) so you have its id and current values, and only include the fields that actually change.',
