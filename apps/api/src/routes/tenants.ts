...
export async function tenantsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/tenants', needsTenants, async (req, reply) => {
  ...
  app.patch<{ Params: { id: string } }>('/tenants/:id', needsTenants, async (req) => {
    const input = parseBody(UpdateTenantInputSchema, req.body);
    return tenantService.update(req.accountId, req.params.id, input);
  });

  app.delete<{ Params: { id: string } }>('/tenants/:id', needsTenants, async (req, reply) => {
    await tenantService.remove(req.accountId, req.params.id);
    return reply.code(204).send();
  });

  app.post<{ Params: { id: string } }>('/tenants/:id/restore', needsTenants, async (req) =>
  ...
  app.post<{ Params: { id: string } }>('/tenants/:id/erase-pii', needsTenants, async (req) =>
