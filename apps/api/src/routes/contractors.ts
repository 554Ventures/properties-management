...
export async function contractorsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.patch<{ Params: { id: string } }>('/contractors/:id', async (req) => {
  ...
  app.delete<{ Params: { id: string } }>('/contractors/:id', async (req, reply) => {
    await contractorService.remove(req.accountId, req.params.id);
    return reply.code(204).send();
  });

  app.post<{ Params: { id: string } }>('/contractors/:id/restore', async (req) =>
    contractorService.restore(req.accountId, req.params.id),
  );

  app.post<{ Params: { id: string } }>('/contractors/:id/jobs', async (req) => {
